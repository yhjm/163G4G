...
<body>
<c:if test="${param.changePage eq 'error' }">
...
<c:if test="${param.update eq 'update' }">
  <script type="text/javascript">
		 alert("修改密码成功");
  </script>
</c:if>
...
<div class="main">
	...
	<div class="content">
		<table class="list" border="2px">
			...
			<c:forEach items="${PageBean.data }"  var = "pagebean">
				...
				<td>
					<a href="JSP/update.jsp?accountId=${pagebean.accountId }">修改</a>
				</td>
			</tr>
			</c:forEach>
	...
	<form id="frm" method="post"  action="commonServlet?cmd=changePage">
	<label class="input-button" >跳转到</label>
		<select name = "pageNum"  id="pageNum" onchange="changeP()">
			<c:forEach begin="1" var = "i"  end="${PageBean.pageTotal }">
	...
	<a style="text-decoration: none;" class="input-button"  href = "commonServlet?cmd=changePage&pageNum=${PageBean.pageNum+1}">下一页</a>
	<a style="text-decoration: none;" class="input-button" href = "commonServlet?cmd=changePage&pageNum=${PageBean.pageNum-1}">上一页</a>
	</form>
</div>
